...
bool DiskMultiMap::createNew(const std::string& filename, unsigned int numBuckets)
{
    ...
    m_bucket = numBuckets;
    firstfreeNode = 0;
    m_file.write(head(0,0,numBuckets),0);   //head structure for DiskMultiMap  0 used, 0 in total
...
bool DiskMultiMap::insert(const std::string& key, const std::string& value, const std::string& context)
{
    ...
    BinaryFile::Offset Offset = keyhash(key,bkt.numbucket);
    ...
    if (firstfreeNode!=0)
    {
        ...
        m_file.write(head(h.numinuse+1,h.numintotal,h.numbucket),0);
    }
    else
    {    start = k;
        m_file.write(head(h.numinuse+1,h.numintotal+1,h.numbucket),0);
    }
...
BinaryFile::Offset DiskMultiMap::keyhash(string key,int bucket)
{
    hash<string> hasher;
    unsigned int rkey = hasher(key);
    Bucket bk;
    head a = head(0,0,0);
    rkey = (((rkey % (bucket))*sizeof(bk))+sizeof(a));
    return rkey;
}
...
DiskMultiMap::Iterator& DiskMultiMap::Iterator::operator++()
{
    ...
    {    offset = d.next;
        while (d.next!=0)
        {
            if (p->m_file.read(d,d.next)&& d.key==s)
                break;
            else
            {
                offset = d.next;
